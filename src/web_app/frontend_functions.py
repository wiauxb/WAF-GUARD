...
def format_directive_table(directive_table: pd.DataFrame) -> pd.DataFrame:
    ...
    # Object columns can contain mixed types (e.g. str + int from fillna(-1)).
    # Cast them all to str so PyArrow can serialise the dataframe without errors.
    for col in directive_table.select_dtypes(include="object").columns:
        directive_table[col] = directive_table[col].astype(str)
