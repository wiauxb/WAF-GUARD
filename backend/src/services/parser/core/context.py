...
class FileContext(Context):
    # PORT CHANGE: config_root was a global os.environ["CONFIG_ROOT"] in the old analyzer.
    # It is passed explicitly here because two configurations can now be parsed
    # concurrently in the same process, and a global would let one parse resolve the
    # other's files. Behaviour is otherwise identical.
    def __init__(self, line_num : int, file_path : str, config_root : str = None):
        super().__init__(line_num)
        self.file_path = file_path
        self.config_root = config_root

    def to_real_path(self):
        conf_path = os.path.join(self.config_root, "conf", "")
