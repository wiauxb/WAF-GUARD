...
class PostgresDB:
    ...
    def add_rule(self, ctx: Context, node_id: int = None):
        ...
        return self.execute(
            "INSERT INTO symboltable (file_path, line_number, node_id) VALUES (%s, %s, %s) RETURNING id;",
            (file_path, line_number, node_id)
            )[0][0]
    
    def add_macrodef(self, name: str, rule_id: int):
        self.execute("INSERT INTO macrodef (name, ruleid) VALUES (%s, %s);", (name, rule_id))
        return name

    def add_macrocall(self, node_id: int, macro_id: int, rule_id: int):
        return self.execute(
            "INSERT INTO macrocall (nodeid, macro_name, ruleid) VALUES (%s, %s, %s) RETURNING id;",
            (node_id, macro_id, rule_id)
            )[0][0]
    ...
    def add(self, directive: Directive):
        ...
        while ptr_ctx is not None:
            if isinstance(ptr_ctx, FileContext):
                ...
                def_id = self.execute("SELECT * FROM macrodef WHERE name = %s;", (ptr_ctx.macro_name,))
                if not def_id:
