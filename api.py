...
async def get_metadata(node_id: str):#FIXME if node_id is not in a macro_call, it will return an empty list
    ...
    cursor.execute("SELECT mc.macro_name, st.file_path, st.line_number FROM macrocall mc, symboltable st WHERE mc.nodeid = %s and mc.ruleid = st.id ORDER BY mc.id DESC", (node_id,))
